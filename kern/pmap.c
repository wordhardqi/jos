...
static void *boot_alloc(uint32_t n) {
  ...
  // cprintf("bool_alloc memory at %x \n", nextfree);
  if (n == 0) {
    return nextfree;
  }
  result = nextfree;
  ...
  if ((size_t)nextfree > KERNBASE + npages * PGSIZE) {
    panic("boot_alloc: out of memory at %x, %x \n", result, npages * PGSIZE);
  }
  return result;
}
...
void mem_init(void) {
  ...
  boot_map_region(kern_pgdir, UPAGES, pages_size, PADDR((void *)(pages)),
                  PTE_U | PTE_P);
  ...
  boot_map_region(kern_pgdir, UENVS, envs_size, PADDR(((void *)(envs))),
                  PTE_U | PTE_P);
  ...
  for (; i < npages; i++) {
    if (i == 0 ||
        (&pages[i] >= pa2page(PADDR((void *)(KERNBASE + IOPHYSMEM))) &&
         &pages[i] < pa2page(PADDR((void *)(KERNBASE + EXTPHYSMEM)))) ||
        (&pages[i] >= pa2page(PADDR((void *)(KERNBASE + EXTPHYSMEM))) &&
         &pages[i] < pa2page(PADDR((void *)(boot_alloc(0))))) ||
        (&pages[i] == pa2page(MPENTRY_PADDR))) {
      pages[i].pp_ref = 1;
      pages[i].pp_link = NULL;
    } else {
      pages[i].pp_ref = 0;
...
void page_free(struct PageInfo *pp) {
  ...
  if (pp->pp_ref != 0) {
    panic("Page at %08x is still referred", pp);
  }
  if (pp->pp_link != NULL) {
    panic("Page at %08x is alrady free", pp);
  }
...
// we installed a Global Descriptor Table (GDT) that effectively disabled
// segment translation by setting all segment base addresses to 0 and limits to
// 0xffffffff. Hence the "selector" has no effect and the linear address always
// equals the offset of the virtual address. In lab 3, we'll have to interact a
// little more with segmentation to set up privilege levels, but as for memory
// translation, we can ignore segmentation throughout the JOS labs and focus
...
pte_t *pgdir_walk(pde_t *pgdir, const void *va, int create) {
  ...
  if (!(pgdir[pdx] & PTE_P)) {
    if (create) {
      struct PageInfo *pg = (struct PageInfo *)page_alloc(ALLOC_ZERO);
      // failed to find free space
      if (!pg)
        return NULL;
      ++pg->pp_ref;
      // create the link to page drectory
...
static void boot_map_region(pde_t *pgdir, uintptr_t va, size_t size,
  ...
  for (i = 0; i < size / PGSIZE; ++i, va += PGSIZE, pa += PGSIZE) {
    ...
    if (!pte) {
      panic("boot_map_region %s,%d  panic", __FILE__, __LINE__);
    }
    *pte = pa | perm | PTE_P;
  }
}
...
void *mmio_map_region(physaddr_t pa, size_t size) {
  ...
  boot_map_region(kern_pgdir,base,aligned_size,pa,perm);
...
int user_mem_check(struct Env *env, const void *va, size_t len, int perm) {
  ...
  int tmp_perm = PTE_P | perm;
  for (; begin < end; begin += PGSIZE) {
    pte_t *p = pgdir_walk(env->env_pgdir, (void *)begin, 0);
    if (p == NULL || (((*p) & tmp_perm) != tmp_perm) || begin > ULIM) {
